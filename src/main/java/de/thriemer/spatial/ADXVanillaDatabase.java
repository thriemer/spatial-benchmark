...
public class ADXVanillaDatabase extends DatabaseAbstraction {
        ...
        if (!sqrNames.containsKey(sqrName)) {
            ...
            String query =
                    """
                            ...
                            {{tableName}}
                            | where geo_point_in_polygon(longitude, latitude, dynamic({{geojson}}))
    ...
    public List<DataPoint> fetchArea(String tableName, Geometry contractPolygon, Geometry filter) {
        ...
        String query = """
                set notruncation;
                {{tableName}}
                | where geo_point_in_polygon(longitude,latitude, geo_intersection_2polygons(dynamic({{geojson1}}), dynamic({{geojson2}})))
